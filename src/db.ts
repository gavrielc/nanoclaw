...
function createSchema(database: Database.Database): void {
  ...
  // Add media_data column for file/image attachments
  try {
    database.exec(`ALTER TABLE messages ADD COLUMN media_data TEXT`);
  } catch {
    /* column already exists */
  }
}
...
export function storeMessageDirect(msg: {
  ...
  db.prepare(
    `INSERT OR REPLACE INTO messages (id, chat_jid, sender, sender_name, content, timestamp, is_from_me, is_bot_message, media_data) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  ).run(
    msg.id,
    msg.chat_jid,
    msg.sender,
...
export function getMessagesSince(
  ...
  // prefix as a backstop for messages written before the migration ran.
  const sql = `
    SELECT id, chat_jid, sender, sender_name, content, timestamp, media_data
    FROM messages
